<script setup>
  import { RouterLink } from 'vue-router';
  import Navbar from '../components/Navbar.vue';
  import Footer from '../components/Footer.vue';

</script>

<template>
  <div class="flex flex-col h-dvh">
    <Navbar />
    <section class="text-center flex flex-col justify-center items-center h-dvh text-white">
      <i class="pi pi-exclamation-triangle text-yellow-600 text-7xl mb-5"></i>
      <h1 class="text-6xl font-bold mb-4">404 Not Found</h1>
      <p class="text-xl mb-5">This page does not exist</p>
      <RouterLink
        to="/"
        class="text-white bg-green-700 hover:bg-green-900 rounded-md px-3 py-2 mt-4">
          Go Back
      </RouterLink>
    </section>
    <Footer  />
  </div>
</template>

